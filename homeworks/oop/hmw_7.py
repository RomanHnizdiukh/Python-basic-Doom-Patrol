...
class Gardener:
    ...
    def poison_pests(self):
        for pest in self.pests:
            if self.pests >= [1]:
                print(f"Poisoning {self.pests} pests")
            else:
                print("There are no pests to poison")
...
gardener = Gardener("Homer", [apple_tree, tomato_bush], "34r")
for _ in range(4):
    gardener.take_care()
gardener.gardener_harvest()
gardener.poison_pests()
print(tomato_bush.all_tomatoes)
print(apple_tree.all_apples)